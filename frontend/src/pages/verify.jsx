...
import { verifyProof } from '../lib/publishproof'
...
class PublishPage extends Component {
  ...
  async handleFormEvent (event) {
    ...
    // verify proof
    const result = await verifyProof(userId, balance, `last ${months} months`)

    if (result === null) {
      window.alert('No, the proof is not valid')
    } else {
      window.alert(`Yes. Verified by ${result}`)
    }
  }
        ...
        <Paper className={classes.paper}>
          <form onSubmit={this.handleFormEvent}>
            ...
            <TextField
              ...
              fullWidth
            />
            maintain an average balance of at least
            <TextField
              name='balance'
              autoComplete='off'
              label='Balance'
